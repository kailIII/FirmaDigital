...
/**
 * Implementacion simple de RawSignatureProcessor usando clases incluidas en el
 * JDK.
 * 
 * @author Ricardo Arguello <ricardo.arguello@soportelibre.com>
 * @version $Revision: 38 $
 */
public class JDKSignatureProcessor implements RawSignatureProcessor {

	private static final String DIGITAL_SIGNATURE_ALGORITHM_NAME = "SHA1withRSA";

	private static final Logger logger = Logger.getLogger(JDKSignatureProcessor.class.getName());
	...
	public boolean verify(InputStream data, byte[] signature, PublicKey publicKey) {
		try {
			Signature sig = Signature.getInstance(DIGITAL_SIGNATURE_ALGORITHM_NAME);
			sig.initVerify(publicKey);
